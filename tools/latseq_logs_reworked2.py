...
def _parse_chunk_worker(
    ...
    with open(file_path_str, "rb") as f:
        if start_byte != 0:
            f.seek(start_byte)
            f.readline()
        while (end_byte == -1 or f.tell() < end_byte) and (
...
def _rebuild_batch_worker(batch_info):
    ...
    for start_event in startpoint_batch:
        try:
            t0 = perf_counter()
            journeys = _SHARED_REBUILDER._rebuild_journeys_from_startpoint(
                start_event
            )
...
class LatSeqLogParser:
    ...
    def __init__(self, filepath: str, max_workers: int | None = None):
        ...
        logger.info(
            f"Parsed {self.event_count} events in {self.__class__.__name__}"
        )
    ...
    def _parse_in_parallel(self):
        ...
        chunks = [
            (
                str(self.filepath),
                i * csz,
                -1 if i == n_workers - 1 else (i + 1) * csz,
                i,
            )
        ...
        logger.info(
            f"Parsing log in parallel using {n_workers} processes across {len(chunks)} chunks"
        )
        ...
        logger.info(
            f"Finished parsing {total_lines_read:,} total log lines across {len(results)} chunks. Building indices..."
        )

        current_line_offset, up_by_src, down_by_src = (
            0,
            defaultdict(list),
            defaultdict(list),
        )
        ...
        self.uplink_events_by_src = {
            k: {
                "events": (evs := sorted(v, key=lambda e: e.ts)),
                "timestamps": [e.ts for e in evs],
            }
            for k, v in up_by_src.items()
        }
        self.downlink_events_by_src = {
            k: {
                "events": (evs := sorted(v, key=lambda e: e.ts)),
                "timestamps": [e.ts for e in evs],
            }
            for k, v in down_by_src.items()
        }


class LatSeqDiagnostics:
    """Diagnostic suite for LatSeq topology, matching, and performance verification."""

    @staticmethod
    def analyze_matching_local_ids(parser: LatSeqLogParser) -> None:
        ...
        downlink_hops = [
            t for t in two_hop_transitions if "Downlink" in t["direction"]
        ]
        uplink_hops = [
            t for t in two_hop_transitions if "Uplink" in t["direction"]
        ]
        other_hops = [
            t
            for t in two_hop_transitions
            if "Downlink" not in t["direction"]
            and "Uplink" not in t["direction"]
        ]

        print("\n" + "=" * 95)
        print(
            " LATSEQ 2-HOP TRANSITION & MATCHING LOCAL-ID MAPPING (A -> B  ==>  B -> C)"
        )
        print("=" * 95)

        def _print_section(title, transitions):
            print(
                f"\n=== {title} ({len(transitions)} transition branches) "
                + "=" * (55 - len(title))
            )
            if not transitions:
            ...
            for t in transitions:
                ...
                print(
                    f"    MATCHED KEYS    : {t['matching_keys']}  <-- Shared keys used by rebuilder"
                )
                print(
                    f"    No-Segmentation : In: {t['no_seg_in']} | Out: {t['no_seg_out']}"
                )
                print()
...
class LatSeqJourneyRebuilder:
    ...
    def rebuild_journeys(self, batch_size: int = 250) -> None:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            futures = [
                executor.submit(_rebuild_batch_worker, b) for b in batches
            ]
            with tqdm(
                total=total,
    ...
    def _find_matching_events_for_journey(self, journey: dict) -> list[Event]:
        ...
        ) or not (prev_ids := last_ev.localIDs):
            return []
    ...
    def _branch_journey_for_multiple_matches(
        self, journeys: list[dict], base_idx: int, matches: list[Event]
    ) -> None:
...
def main():
    ...
    mode_group.add_argument(
        ...
        "--journeys",
        action="store_true",
        help="Reconstruct packet journeys and export to JSON.",
    )
    mode_group.add_argument(
        "-m",
        "--map-local-ids",
        action="store_true",
        help="Analyze and display 2-hop localIDs used for matching without rebuilding journeys.",
    )

    out_group = parser.add_argument_group("Output Options")
    out_group.add_argument(
        "-o", "--output-file-path", help="Path to output journeys JSONL file."
    )
    out_group.add_argument(
        "--stdout",
        action="store_true",
        help="Enable writing JSON output to stdout.",
    )

    diag_group = parser.add_argument_group("Diagnostics & Debugging")
    ...
    if args.journeys:
        rebuilder = LatSeqJourneyRebuilder(
            processor,
